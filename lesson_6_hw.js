...
function stringToArray(str) {
    ...
    // Видалити пробіли з кожного слова
    const cleanWords = words.map(word => word.trim());

    // Відфільтрувати порожні слова
    const nonEmptyWords = cleanWords.filter(word => word !== '');

    // Сортувати слова за алфавітним порядком
    const sortedWords = nonEmptyWords.sort((a, b) => a.localeCompare(b, 'uk'));
...
function removeDuplicatesAndSort(arr) {
    const uniqueArray = Array.from(new Set(arr)); // Видаляємо елементи масива, що дублюються

    return uniqueArray.sort((a, b) => a - b); // Сортуємо за зростанням
}
...
function createArray(element) {
    ...
    while (result[result.length - 1] + element <= 30) {
        result.push(result[result.length - 1] + element);
    }
